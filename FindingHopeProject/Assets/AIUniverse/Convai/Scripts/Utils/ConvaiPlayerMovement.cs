    ...
    public class ConvaiPlayerMovement : MonoBehaviour
    {
        ...
        private void MovePlayer()
        {
            ...
            if (canMove && !EventSystem.current.IsPointerOverGameObject())
            {
                ...
                horizontalMovement = forward * curSpeedX + right * curSpeedY;

                if (_characterController.isGrounded && Input.GetButton("Jump")) _moveDirection.y = jumpSpeed;
            }
        ...
        private void RotatePlayerAndCamera()
        {
            ...
            // Vertical rotation
            _rotationX -= Input.GetAxis("Mouse Y") * lookSpeed / 2;
            _rotationX = Mathf.Clamp(_rotationX, -lookXLimit, lookXLimit);
            playerCamera.transform.localRotation = Quaternion.Euler(_rotationX, 0, 0);

            // Horizontal rotation
            float rotationY = Input.GetAxis("Mouse X") * lookSpeed / 2;
